...
# Cost pressure is not a dedicated LM label, so we derive it from LM-scored
# negative / constraining vocabulary restricted to cost-like terms.
COST_TERMS = {
    "cost",
    "costs",
    "expense",
...
# Small fallback so the app still runs before the real LM CSV is added.
FALLBACK_LM_ROWS = [
...
class TwoHiddenLayerFinancialSignalModel:
    """
    Two-hidden-layer model whose inputs come from Loughran-McDonald categories.

    The hidden layers remain, but the evidence feeding them is now LM dictionary
    matches across sentence lemmas instead of hand-authored phrase weights.
    """
    ...
    def build_features(self, sentence: dict) -> SentenceFeatureBundle:
        ...
        for lemma in lemmas:
            ...
            matched_categories = [name for name, value in scores.items() if value > 0]
